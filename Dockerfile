...
RUN perl Makefile.PL --config=/usr/local/cwb-3.4.16/bin/cwb-config && make && make test && make install
RUN mkdir /app /cwb/registry /cwb/data
ENV CORPUS_REGISTRY="?/cwb/registry:?/usr/local/cwb-3.4.16/share/cwb/registry/"
ADD . /app
WORKDIR /app
