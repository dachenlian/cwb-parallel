...
sub new {
  ...
  my $self = {
              ...
              VERBOSE => 0,     # enable/disable progress messages
...
sub new {
  ...
  my $self = {                  # create and initialise object
              ...
              AUTONULL => 0,    # auto-declare null attributes for unkown XML tags
...
sub memory {
  my ($self, $mem) = @_;
  croak "CWB::Encoder: memory limit ($mem) must be positive integer number (aborted).\n"
    unless $mem =~ /^[1-9][0-9]*$/;
  $self->{MEMORY} = $mem;
}
...
separate argument to B<encode_options> because they will not be
...
Copyright (C) 2002-2020 Stefan Evert [http::/purl.org/stefan.evert]
