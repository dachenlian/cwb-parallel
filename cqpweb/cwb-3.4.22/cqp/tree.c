...
print_evaltree(int envidx, Evaltree etptr, int indent)
{
  ...
  if(etptr != NULL) {
    ...
    case meet_union:
      ...
      case cooc_meet:
        printf("Meet <%d/%d, %s>", etptr->cooc.lw, etptr->cooc.rw,
               etptr->cooc.struc ? etptr->cooc.struc->any.name : "words");
        break;
      case cooc_union:
        printf("Union ");
        break;
 ...
 *
 * @param etptr  The evaluation tree to delete. This is a pointer to a pointer since
...
free_evaltree(Evaltree *tree_ptr)
{
  if (*tree_ptr) {
    ...
    switch ((*tree_ptr)->node.type) {
       ...
       * it is quite OK to call this function on a NULL pointer because of the containing "if". */
    ...
    case node:
      ...
      case re_disj:
        ...
        assert((*tree_ptr)->node.left != NULL && (*tree_ptr)->node.right != NULL);
        /* dleete left child, then right child, then current op node */
    ...
    case meet_union:
      assert((*tree_ptr)->cooc.left != NULL && (*tree_ptr)->cooc.right != NULL);
    ...
    default:
      assert(0 && "Invalid node type: should not be reached because every possibility was tested");
      break;
    }
  }
}
 ...
 *
 * At the moment, this involves simply setting the Constrainttree to NULL.
 *
