 ...
 * It's swtched on in the SearchPattern parser rule when it gets to RegWordfExpr
...
in_CorpusCommand(char *id, CorpusList *cl)
{
  if (cl == NULL)
    return NULL;
  else if (subcorpus_name_is_qualified(id)) {
...
/**
 * Print a given string, with redirection to a file/pipe specified by a supplied redirect structure.
 *
 * It also implements the \t, \n, \r escape sequences.
 *
 * The parser grammar makes only tiny use of this function.
 *
 * @param s   String to print
 * @param rd  Redirection specifier.
 */
void
...
/**
 * Execute the commands contained in a CQP script file.
 *
 ...
 *                a count, or as a percerntage of existing matches.
 */
void
...
/**
 * Implememnts the query part of a StandardQuery, that is, the
 * part that contains the token-level regular expression,
 * global constraint, and within clause
 * (and not any leading modifiers or trailing  alignment contraints, cut commands, etc.)
 *
...
{
  int searchstr_length; /* var that receives the length of searchstr, which we don't use. */
  ...
  if (generate_code) {
    ...
    if (!check_labels(CurEnv->labels)) {
      ...
      /* create a regex string from the current eval tree */
      searchstr = (char *)evaltree2searchstr(CurEnv->evaltree, &searchstr_length);
      ...
      /* if searchstr is NEITHER empty NOR just a string of spaces, compule it to a DFA */
...
/**
 * Create a constraint tree node that contains a function.
 *
 * @param f_name   Name of the function.
 * @param apl      Parameter list for the function.
 * @return         New Constrainttree object, of type function,
 *                 with the supplied parameter list.
 */
Constrainttree
FunctionCall(char *f_name, ActualParamList *apl)
{
  ...
  if (generate_code) {
    ...
    /* set len to the length of the linked list of arguments */
    ...
    if (predef >= 0) {
      ...
      if (n_args != builtin_function[predef].nr_args) {
        ...
        res->func.args = apl;
        res->func.nr_args = n_args;
      }
    }
    else if (NULL != (attr = cl_new_attribute(query_corpus->corpus, f_name, ATT_DYN))) {
      /* no predef function found: try to get a reference to the named function as a DynamicAttribute */
      NEW_BNODE(res);
      res->type = func;
      res->func.predef = -1;
      ...
      /* no predef function found, and no DynamicAttribute could be created */
...
Evaltree
do_MeetStatement(Evaltree left, Evaltree right, Context *context)
{
  Evaltree ev;
...
do_undump(char *corpname, int extension_fields, int sort_ranges, struct InputRedir *rd)
{
  ...
  if (abort) {                         /* if an error was detected, don't create the named query result */
    close_input_stream(rd);
    drop_temp_corpora();
    return 0;
  }
