...
Showing:          SHOW_SYM              { show_corpora_files(UNDEF); }
                | SHOW_SYM ID           {
...
OptionSetCmd:     SET_SYM ID VarValue   {
                ...
                | SET_SYM 
                  CID 
                  FIELD 
                  FIELD                 { do_set_target($2, $3, $4); }
                | SET_SYM 
                  CID 
                  FIELD 
                  NULL_SYM              { do_set_target($2, $3, NoField); }   /* erase field */
                | SET_SYM 
                  CID 
                  FIELD 
                  SEARCH_STRATEGY       {
...
SearchPattern:                          {
                                          if (generate_code) { 
                                            CurEnv->match_label = labellookup(CurEnv->labels, "match", LAB_DEFINED, 1);
                                          }
                                        }
                  RegWordfExpr          { 
                                          within_gc = 1; 
                                          if (generate_code)
                                            CurEnv->matchend_label = labellookup(CurEnv->labels, "matchend", LAB_DEFINED, 1);
                                        }
                  GlobalConstraint      { within_gc = 0; }
                  SearchSpace           { do_SearchPattern($2, $4); }
                ;

                  /* NB RegWordfExpr is a misnomer: it's a token-level regular expression */
...
RelExpr:          RelLHS 
                ...
                | RelLHS 
                  ...
                  STRING
                  OptionalFlag          {
                                          /* operators for multi-valued attributes */
                                          if ($2 & OP_NOT) 
...
Macro:            OptDEFINE
                  ...
                  MultiString           {
                                          ...
                                          cl_free($7);  /* don't forget to free the allocated strings */
                                          cl_free($3);
                                        }
                | OptDEFINE
                  MACRO_SYM
                ...
                | OptDEFINE
                  ...
                  STRING                {
                                          load_macro_file($4);
                                          cl_free($4);  /* don't forget to free the allocated string */
                                        }
                ;
 ...
 * except OptionalFIELD for some reason...)
 */
