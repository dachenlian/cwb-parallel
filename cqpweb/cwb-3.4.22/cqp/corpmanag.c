 ...
 * @param  ft  The field type to count (Match/Keyword/Target).
 * @return     The number of values of the specifed field-type.
 *
 *
static int
NrFieldValues(CorpusList *cl, FieldType ft)
{
  ...
  if (cl) {
    switch (ft) {
// note : shouldn't MatchEndField yield the same as MatchField?
    case MatchField:
      nr_items = cl->size;
      break;
 ...
 *                       if it's NULL, then the dropped corpus was at the end of the list,
 *                       or not found on the part of the list searched. See drop_temp_corpora().
 */
CorpusList *
...
//  CorpusList *cl, *prev, *del;
   ...
   * time
   */
  /* I think that the new loop below now does what was intended by the comment above - AH 2020-02-02. */
...
//  }
  ...
  CorpusList *curr, *prev;

  for (prev = NULL, curr = corpuslist ; curr ; curr = prev->next)
    if (TEMP == curr->type)
      prev = dropcorpus(curr, prev);
    else
      prev = curr;
}
 ...
 *
 * @param cl                 REpresentor of the subcorpus/NQR  whose path we're building.
...
/**
 * Checks whether a name is available for use as a subcorpus ID.
 *
 * This is NOT the case if the name is in use for a system corpus.
 *
 * (The function name is pretty deceptive, "valid" implies well formed,
 * but that is not what is tested. "Available" would be a better prefix.)
 *
 * Function seems not to be in use.
 *
 * @param corpusname
 * @return
static Boolean
...
{
  return !findcorpus(corpusname, SYSTEM, 0);
}
 */
...
{
  show_corpora_files_backend(ALL, 1);
}
