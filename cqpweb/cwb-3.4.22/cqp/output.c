...
open_stream(struct Redir *rd, CorpusCharset charset)
{
  ...
  if (rd->name) {
    ...
    mode = cl_streq(rd->mode, "a") ? CL_STREAM_APPEND : CL_STREAM_WRITE;
...
corpus_info(CorpusList *cl)
{
  ...
  if (cl->type == SYSTEM) {
    ...
    outfh = stream_ok ? rd.stream : stdout; /* use pager, or simply print to stdout if it fails */
    ...
    if (NULL == cl->corpus->info_file)
      fprintf(outfh, "No further information available about %s\n", cl->name);
    else if (NULL == (fh = cl_open_stream(cl->corpus->info_file, CL_STREAM_READ_BIN, CL_STREAM_MAGIC)))
      fprintf(outfh, "No further information available about %s\n", cl->name);
      /* most of the time this is NOT a problem - it just means the
       * default HOME/.info has not been created. Just another way that there can be no more info. */
  ...
  if (!cl->mother_name)
    ...
    /* print mother's info; if the mother is not loaded, we just have to print an error */
    if (NULL == (mother = findcorpus(cl->mother_name, SYSTEM, 0)))
      cqpmessage(Info,
