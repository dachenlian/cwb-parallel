                             ...
                             int restrictor_size)
{
  ...
  if (cl_streq(regstr, ".*")) {
    ...
    if (n_forms_matched == range) {
      ...
      /* Some matching forms have been found. */
               ...
               int deliver_strings)
{
  ...
  case float_leaf:
    ...
    break;
    // TODO possible problem, this goes straight to the "not reached" assertion. Should there be a return True here?
...
/**
 * This function's name is totally useless. It's called within simulate_dfa and check_alignment_constraints.
 */
static void
    ...
    while ((i < matchlist->tabsize) && ((*cut) != 0) && EvaluationIsRunning) {
      ...
      if (debug_simulation)
        fprintf(stderr, "Looking at matchlist element %d (cpos %d)\n  range[rp=%d]=[%d,%d]\n",
                i, matchlist->start[i],
                rp,
                rp < evalenv->query_corpus->size ? evalenv->query_corpus->range[rp].start : -1,
                rp < evalenv->query_corpus->size ? evalenv->query_corpus->range[rp].end   : -1);

      if (rp >= evalenv->query_corpus->size || matchlist->start[i] < evalenv->query_corpus->range[rp].start) {
        /* case 1
         * - no match possible
         */
        matchlist->start[i] = -1;
                ...
                for (p = 0 ; p < evalenv->dfa.Max_Input && (winner < 0 || evalenv->matching_strategy == longest_match) ; p++) {
                  ...
                  if (target_state != evalenv->dfa.E_State) {
                    ...
                    if ( ((effective_cpos <= boundary) || (lookahead_constraint && (effective_cpos == boundary+1))) && strict_regions_ok ) {
                       ...
                       */
                      if (state == start_state && start_transition >= 0 && first_transition_traversed == 0) {
                       ...
                       */
                        ...
                        transition_valid = eval_constraint(condition, effective_cpos, reftab_vector[state], reftab_target_vector[target_state]);
 ...
 * since CQP is, technically, only *simulating* the machine described by the DFA!)
 *
      ...
      if ((state = evalenv->dfa.TransTable[0][p]) != evalenv->dfa.E_State) {
        ...
        if (match_first_pattern(&(evalenv->patternlist[p]), &matchlist, evalenv->query_corpus)) {
          ...
          if (matchlist.tabsize > 0) {
            ...
            /* eep > 0 iff there are alignment constraints */
            maxresult = (cut <= 0 || eep > 0) ? -1 : cut;
 ...
 */
static int
meet_mu(Matchlist *list1, Matchlist *list2, int lw, int rw, Attribute *struc)
{
