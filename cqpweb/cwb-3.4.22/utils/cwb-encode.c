...
/*    fprintf(stderr, "  -r <dir>  set registry directory (for -C flag)\n"); */
  ...
  fprintf(stderr, "  -9        discard XML tags for all undeclared s-attributes (auto null-attributes)\n");
...
encode_parse_options(int argc, char **argv)
{
  ...
  while((c = getopt(argc, argv, "p:P:S:V:0:f:t:F:d:R:U:Bsb:c:Cx9vqhD")) != EOF)
    switch(c) {
    ...
    case 'x':
      ...
      /* -9: discard XML tags for all undeclared s-attributes (i.e. auto-declare them as null attributes) */
    case '9':
      auto_null++;
      break;
      ...
      if (buf[0] == '<') {
        ...
        if (i > k) {
          ...
          if ((s_att_ix = s_att_builder_find(&buf[k])) >= 0) {
            ...
            /* no appropriate s-attribute declared -> insert tag literally (unless auto-null option -9 was specified) */
            if (auto_null) {
              handled = 1; /* mark line as handled (i.e. ignored) */
              if (!silent) {
                fprintf(stderr, "undeclared s-attribute <%s> will be ignored (implicit '-0 %s', ", &buf[k], &buf[k]);
                encode_print_input_lineno();
                fprintf(stderr, ").\n");
              }
              s_att_declare(&buf[k], directory, /*annot*/ 0, /*null*/ 1); /* auto-declare as null attribute */
